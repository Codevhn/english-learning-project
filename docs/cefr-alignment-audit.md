...
**Estado — Fase B (listening) y Fase C (speaking): RESUELTAS.** `packages/database/supabase/seed/007_listening_speaking.sql` agrega dos módulos nuevos al final del currículo A1: módulo 14 "Comprensión Auditiva" (5 lecciones, 40 ejercicios tipo `listening`, modos de selección múltiple y dictado) y módulo 15 "Producción Oral" (5 lecciones, 40 ejercicios tipo `speaking`, lectura en voz alta con autoevaluación). Se construyeron componentes de UI reales: `Listening.tsx` (Web Speech API `speechSynthesis` para TTS) y `Speaking.tsx` (Web Speech API `SpeechRecognition` para STT, con coincidencia difusa por distancia de Levenshtein y fallback de autoevaluación en navegadores sin soporte). Todo el contenido nuevo reutiliza vocabulario y gramática ya enseñados en los módulos 1-13 — ningún ejercicio introduce conceptos nuevos. Validado ejecutando el esquema real + todos los seeds (001→007) contra una instancia de Postgres limpia (15 módulos, 70 lecciones, 562 ejercicios, re-ejecución idempotente confirmada).
